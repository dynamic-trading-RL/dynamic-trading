...
def maxAction(q_value, state, bounds, b, optimizers, optimizer=None):
    ...
    if b == 0:
        ...
        if optimizer is None:
            ...
            res = [res1.x[0], res2.x[0], res3.x[0], res4[0], res5.x[0]]
            ...
            optimizers._shgo += 1
            res = shgo(fun, bounds=bounds).x[0]

        elif optimizer == 'dual_annealing':
            optimizers._dual_annealing += 1
            res = dual_annealing(fun, bounds).x[0]

        elif optimizer == 'differential_evolution':
            optimizers._differential_evolution += 1
            res = differential_evolution(fun, bounds).x[0]

        elif optimizer == 'brute':

            optimizers._brute += 1
            res = brute(fun, ranges=bounds,
                            Ns=200,
                            finish=None)[0]

        elif optimizer == 'local':
            optimizers._local += 1
            res = minimize(fun, x0=np.array([0]), bounds=bounds).x[0]
